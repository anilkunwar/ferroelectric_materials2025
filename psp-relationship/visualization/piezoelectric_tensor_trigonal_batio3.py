...
if 'piezo_input' not in st.session_state:
    st.session_state.piezo_input = """0 0 0 0 -3.582 -1.673
-1.673 1.673 0 -3.582 0 0
-2.156 -2.156 -3.364 0 0 0"""
if 'stiffness_input' not in st.session_state:
    st.session_state.stiffness_input = """224 62 28 -40 0 0
62 224 28 40 0 0
28 28 194 0 0 0
-40 40 0 31 0 0
0 0 0 0 31 -40
...
if 'material_name' not in st.session_state:
    st.session_state.material_name = "BaTiO3 (Trigonal)"
...
\boldsymbol{\sigma} = \begin{bmatrix} 0 & 0 & 0 & 0 & 0 & 1 \end{bmatrix}^T n_i n_j
$$
...
\mathbf{n} = \begin{bmatrix} l \\ m \\ n \end{bmatrix}, \quad
\mathbf{a} = \begin{bmatrix} l^2 \\ m^2 \\ n^2 \\ 2mn \\ 2nl \\ 2lm \end{bmatrix}
$$
...
This formulation is general and applies to **any crystal symmetry**, including **trigonal BaTiO₃**.

---

### Unit Conversion Note:

- \( \mathbf{S} \): Compliance matrix, in units of GPa\(^{-1}\)
- \( \mathbf{d} = \mathbf{e} \mathbf{S} \): Yields units of \(10^{-9}\) C/N
- To express \( d_{33}^{\text{eff}} \) in **picoCoulombs per Newton (pC/N)**, multiply by \(10^{12}\):
